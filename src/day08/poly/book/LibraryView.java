...
//        sc = new Scanner(System.in);
    ...
    private static Gender inputGender() {
        while (true) {
            String inputGender = Utility.input("# 성별(M/F): ");
            Gender gender;
    ...
    private static void selectMenu() {
        ...
        switch (menuNum) {
            ...
            case "3":
                // 사용자에게 검색어를 입력받는다.
                String keyword = Utility.input("# 검색어: ");
            ...
            case "4":
                ...
                String rentNum = Utility.input("- 대여할 도서 번호 입력 :");
                RentStatus rentStatus = repository.rentBook(Integer.parseInt(rentNum));
                if(rentStatus==RentStatus.RENT_SUCCESS_WITH_COUPON) {
                    System.out.println("# 성공적으로 요리책 쿠폰발급과 함께 대여되었습니다.");
                } else if(rentStatus == RentStatus.RENT_SUCCESS) {
                    System.out.println(" #도서가 성공적으로 대여되었습니다.");
                } else {
                    System.out.println("# 도서 대여가 실패했습니다.");
                }
                break;

            case "5":
                String aKeyword = Utility.input("# 검색어: ");
                String[] searchAuthorBook = repository.searchAuthorBook(aKeyword);
                if (searchAuthorBook.length > 0) {
                    System.out.printf("\n======== [%s] 검색 결과 =========\n", aKeyword);
                    for (String info : searchAuthorBook) {
                        System.out.println(info);
                    }
                } else {
                    System.out.println("\n# 검색 결과가 없습니다.");
